...
def filtrar_datos_google(df, data_type):
    # El nuevo GoogleFinanceScraper devuelve un DataFrame con los nombres de datos en el índice
    # en lugar de en una columna 'Datos'
    if df.empty:
        return pd.DataFrame()
    ...
    # Verificar si el DataFrame tiene columna 'Datos' (datos importados)
    if 'Datos' in df.columns:
        # Filtrar datos importados que ya tienen la estructura correcta
        # Limpiar los prefijos de los nombres en la columna 'Datos' si existen
        df_clean = df.copy()
        df_clean['Datos'] = df_clean['Datos'].str.replace(r'^(Income_|Balance_|Cashflow_)', '', regex=True)
        
        # Filtrar por los items específicos del tipo de dato
        indices_existentes = df_clean[df_clean['Datos'].isin(items_filtro)]
        
        # Si no encontramos datos específicos, intentar buscar cualquier dato que contenga las palabras clave
        if indices_existentes.empty:
            # Buscar coincidencias parciales (sin distinguir mayúsculas/minúsculas)
            for item in items_filtro:
                partial_matches = df_clean[df_clean['Datos'].str.contains(item, case=False, na=False)]
                if not partial_matches.empty:
                    indices_existentes = pd.concat([indices_existentes, partial_matches])
            
            # Eliminar duplicados
            if not indices_existentes.empty:
                indices_existentes = indices_existentes.drop_duplicates()
        
        # Si aún no hay coincidencias, mostrar datos filtrados por prefijo del tipo
        if indices_existentes.empty:
            prefix_map = {
                'balance': 'Balance_',
                'cashflow': 'Cashflow_', 
            ...
            if data_type in prefix_map:
                ...
                prefix_matches = df[df['Datos'].str.startswith(prefix, na=False)]
                if not prefix_matches.empty:
                    # Limpiar los prefijos para mostrar
                    indices_existentes = prefix_matches.copy()
                    indices_existentes['Datos'] = indices_existentes['Datos'].str.replace(f'^{prefix}', '', regex=True)
                else:
                    # Para depuración, devolver algunos datos si existen
                    if not df_clean.empty:
                        indices_existentes = df_clean.head(5)  # Mostrar los primeros 5 para depuración
        
        return indices_existentes
    else:
        # Filtrar datos en vivo por índice (con o sin prefijos)
        indices_existentes = []
        
        # Buscar tanto con prefijos como sin prefijos
        for item in items_filtro:
            # Buscar el item exacto
            if item in df.index:
                indices_existentes.append(item)
            # Buscar con prefijos (Income_, Balance_, Cashflow_)
            for prefix in ['Income_', 'Balance_', 'Cashflow_']:
                prefixed_item = prefix + item
                if prefixed_item in df.index:
                    indices_existentes.append(prefixed_item)
        
        if indices_existentes:
            filtered_df = df.loc[indices_existentes].copy()
            # Convertir el índice en una columna 'Datos' para mantener compatibilidad
            filtered_df.reset_index(inplace=True)
            # Limpiar los prefijos de los nombres para mostrar solo el nombre limpio
            filtered_df['index'] = filtered_df['index'].str.replace(r'^(Income_|Balance_|Cashflow_)', '', regex=True)
            filtered_df.rename(columns={'index': 'Datos'}, inplace=True)
            return filtered_df
        else:
            # Si no hay coincidencias, devolver DataFrame vacío con estructura correcta
...
def filtrar_datos_yahoo_importados(yahoo_df, tipo_dato):
    """
    Filtra los datos de Yahoo Finance importados según el tipo de dato solicitado.
    Esta función no hace llamadas a la API, solo filtra los datos ya importados.
    """
